...
class _OtpScreenState extends ConsumerState<OtpScreen> {
  ...
  void _onDigitChanged(int index, String value) {
    ...
    // Support paste: distribute digits across boxes
    if (value.length > 1) {
  ...
  Future<void> _submit() async {
    ...
    final success = await ref.read(otpFlowProvider.notifier).verifyOtp(otp);
    if (!mounted) return;

    if (success) {
      // Router redirect handles where to go based on shop status.
  ...
  Future<void> _resend() async {
    ...
    _focusNodes.first.requestFocus();
    await ref.read(otpFlowProvider.notifier).resendOtp();
    _startCountdown();
  }
  ...
  Widget build(BuildContext context) {
    ...
    final isLoading = otpState is OtpFlowVerifying;
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Text('Enter the 6-digit code', style: AppTypography.headlineMedium),
